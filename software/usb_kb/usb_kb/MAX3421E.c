...
{
	BYTE read;
	int returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &data, 1, &read, 0);
	if(returncode < 0)
	{
		printf("error");
	}
...
void MAXreg_wr(BYTE reg, BYTE val)
{
	...
	array[0] = reg+2;
	array[1] = val;
	int returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 2, array, 0, NULL, 0);
	if(returncode < 0)
		printf("error");
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data)
{
	...
	int returncode = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes+1, array, 0, NULL, 0);
	if(returncode < 0)
	{
		printf("error");
	}
...
{
	  BYTE read;
	  int returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &read, 0);
	  if(returncode < 0)
	  {
		  printf("error");
	  }
...
{
	int returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes+1, data, 0);
	if(returncode < 0)
	{
		printf("error");
	}
